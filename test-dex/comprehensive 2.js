...
let wireLib, factory, wbnb, maker, taker, crss, mock, mock2, farm, repay, farmLib, xcrss, referral, cCrss;
...
describe("====================== Stage 1: Deploy contracts ======================\n".yellow, async function () {
  it("Main contracts are deployed.\n".green, async function () {
    ...
    // cCrss Deployment.
    cCrss = await deployCCrss(owner, farm.address);
    const cCrssAddr = cCrss.address;
    consoleLogWithTab(`cCrss deployed at: ${cCrss.address}`);
    ...
    tx = crss.connect(owner).wire(xCrssAddr, makerAddr);
    (await tx).wait();
    console.log("\tCrss token was wired: xCrss - O - maker");

    tx = crss.connect(alice).wire(xCrssAddr, makerAddr);
    expectReverted(tx);
    console.log("\tAlice couldn't wire nodes");

    tx = maker.connect(owner).wire(crssAddr, takerAddr);
    (await tx).wait();
    console.log("\tmaker was wired: crss - O - taker");

    tx = taker.connect(owner).wire(makerAddr, farmAddr);
    (await tx).wait();
    console.log("\ttaker was wired: maker - O - farm");

    tx = farm.connect(owner).wire(takerAddr, factoryAddr);
    (await tx).wait();
    console.log("\tfarm was wired: taker - O - factory");

    tx = factory.connect(owner).wire(farmAddr, xCrssAddr);
    (await tx).wait();
    console.log("\tfactory was wired: farm - O - xCrss");

    tx = xCrss.connect(owner).wire(factoryAddr, crssAddr);
    (await tx).wait();
    console.log("\txCrss was wired: factory - O - crss");
    ...
    tx = crss.connect(alice).setNode(NodeTypes.indexOf("Token"), crssAddr, zero_address);
    expectReverted(tx);
    ...
    // (await tx).wait();
    // console.log("\tRepay was set to the node chain");
    ...
    tx = crss.connect(alice).setFeeStores(feeStores, zero_address);
    expectReverted(tx);
...
describe(
  "====================== Stage 3: Test Dex functionalities. ======================\n".yellow,
  async function () {
...
// describe(
//   "====================== Stage 4: Dex functionalities (SWAP). ======================\n".yellow,
//   async function () {
//     it("swapExactTokensForTokens function has checked.\n".green, async function () {
//       let ownerCrssBalance = await crss.balanceOf(owner.address);
//       consoleLogWithTab(`Owner CRSS balance before swap: ${utils.formatEther(ownerCrssBalance)}`);
//       let ownerMockBalance = await mock.balanceOf(owner.address);
//       consoleLogWithTab(`Owner MCK balance before swap: ${utils.formatEther(ownerMockBalance)}`);

//       let block = await ethers.provider.getBlock("latest");
//       await crss.approve(taker.address, utils.parseEther("1"));
//       console.log("\tSwap 1 CRSS to MCK tokens.");
//       await taker.swapExactTokensForTokens(
//         utils.parseEther("1"),
//         0,
//         [crss.address, mock.address],
//         owner.address,
//         block.timestamp + 1000
//       );
//       await expectNotReverted(tx);

//       ownerCrssBalance = await crss.balanceOf(owner.address);
//       consoleLogWithTab(`Owner CRSS balance after swap: ${utils.formatEther(ownerCrssBalance)}`);
//       ownerMockBalance = await mock.balanceOf(owner.address);
//       consoleLogWithTab(`Owner MCK balance after swap: ${utils.formatEther(ownerMockBalance)}`);
//     });

//     it("swapTokensForExactTokens function has checked.\n".green, async function () {
//       let ownerCrssBalance = await crss.balanceOf(owner.address);
//       consoleLogWithTab(`Owner CRSS balance before swap: ${utils.formatEther(ownerCrssBalance)}`);
//       let ownerMockBalance = await mock.balanceOf(owner.address);
//       consoleLogWithTab(`Owner MCK balance before swap: ${utils.formatEther(ownerMockBalance)}`);
...
//       console.log("\tBuy 0.1 MCK from CRSS tokens.");
//       await crss.approve(taker.address, utils.parseEther("2"));
//       tx = taker.swapTokensForExactTokens(
...
//     it("swapExactETHForTokens function has checked.\n".green, async function () {
//       let ownerCrssBalance = await crss.balanceOf(owner.address);
//       consoleLogWithTab(`Owner CRSS balance before swap: ${utils.formatEther(ownerCrssBalance)}`);

//       let block = await ethers.provider.getBlock("latest");

//       console.log("\tBuy CRSS token with 0.1 BNB.");
//       tx = taker.swapExactETHForTokens(
//         utils.parseEther("0"),
//         [wbnb.address, crss.address],
//         owner.address,
//         block.timestamp + 1000,
//         {
...
//     it("swapTokensForExactETH function has checked.\n".green, async function () {
//       let ownerCrssBalance = await crss.balanceOf(owner.address);
//       consoleLogWithTab(`Owner CRSS balance before swap: ${utils.formatEther(ownerCrssBalance)}`);

//       let block = await ethers.provider.getBlock("latest");

//       console.log("\tBuy 0.1 BNB with CRSS tokens.");
//       tx = taker.swapTokensForExactETH(
//         utils.parseEther("0.1"),
//         utils.parseEther("10000"),
...
//     it("swapExactTokensForETH function has checked.\n".green, async function () {
//       let ownerCrssBalance = await crss.balanceOf(owner.address);
//       consoleLogWithTab(`Owner CRSS balance before swap: ${utils.formatEther(ownerCrssBalance)}`);

//       let block = await ethers.provider.getBlock("latest");

//       console.log("\tSwap 1 CRSS to BNBs.");
//       tx = taker.swapExactTokensForETH(
//         utils.parseEther("1"),
//         0,
//         [crss.address, wbnb.address],
//         owner.address,
...
//     it("swapETHForExactTokens function has checked.\n".green, async function () {
//       let ownerCrssBalance = await crss.balanceOf(owner.address);
//       consoleLogWithTab(`Owner CRSS balance before swap: ${utils.formatEther(ownerCrssBalance)}`);

//       let block = await ethers.provider.getBlock("latest");

//       console.log("\tBuy 1 CRSS with BNBs.");
//       tx = taker.swapETHForExactTokens(
//         utils.parseEther("1"),
//         [wbnb.address, crss.address],
//         owner.address,
//         block.timestamp + 1000,
...
async function mintBlocks(blocks) {
  for (let n = 0; n < blocks; n ++) {
    await mock.connect(owner).approve(owner.address, utils.parseEther("1") ); // 1 wei
  }
...
async function showBranchInfo(pid, _branch) {
  ...
  console.log("\t- %s, sumAccumulated: %s, accPerShare: %s", subPool1_name, subPool1.sumAccumulated, subPool1.accPerShare);
  if (subPool2_name != undefined)
  console.log("\t- %s, sumAccumulated: %s, accPerShare: %s", subPool2_name, subPool2.sumAccumulated, subPool2.accPerShare);
}
...
async function showUserInfo(pid, _user) {
  ...
  let collectOption, deposit, accRewards, totalVest, totalWithdrawableVest, pendingCrss, rewardPayroll, lpBalance, crssBalance;
  [collectOption, deposit, accRewards, totalVest, totalWithdrawableVest, pendingCrss, rewardPayroll, lpBalance, crssBalance] = await farm.getUserState(pid, _user.address);

  console.log(`\t= %s, option: %s, depo: %s, accum: %s, vest: %s, wVest: %s`, 
  _user.name, CollectOptions[collectOption], deposit, accRewards, totalVest, totalWithdrawableVest);
  console.log(`\t\tpending: %s, payroll: %s, lp: %s %s, crss: %s`,
  pendingCrss, rewardPayroll, lpBalance, symbol, crssBalance);

  return { symbol: symbol, deposit: deposit, accReward: accRewards, tVest: totalVest, 
    wVest: totalWithdrawableVest, pending: pendingCrss, payroll: rewardPayroll, lp: lpBalance, crss: crssBalance };
}
...
async function getPoolConstants(pid, caller) {
  ...
  if (pid == 0) { 
    symbol = "Crss";
  } else if (pid == pid_compensation) {
    symbol = await cCrss.symbol();
  } else {
...
async function switchCollectOption(pid, caller, newOption) {
  ...
  async function deposit(pid, caller, amount, revert) {
    ...
    if (amount > weiToEth(lp0) ) {
      amount = weiToEth(lp0) * 0.99;
...
async function vestAccumulated(pid, caller) {
  ...
  let user = await farm.userInfo(pid, caller.address);
  console.log("\t%s is going to vest their pending + accumulatedRewards rewards %s %s".yellow, caller.name, user.accumulatedRewards, symbol);
  await showPoolInfo(pid);
...
async function compoundAccumulated(pid, caller) {
  ...
  let user = await farm.userInfo(pid, caller.address);
  console.log("\t%s is going to compound their pending + accumulated rewards %s %s".yellow, caller.name, user.accumulatedRewards, symbol);
  await showPoolInfo(pid);
...
async function harvestAccumulated(pid, caller) {
  ...
  let user = await farm.userInfo(pid, caller.address);
  console.log("\t%s is going to claim their pending + accumulated reward %s %s.".yellow, caller.name, user.accumulatedRewards, symbol);
  await showPoolInfo(pid);
  ...
  console.log(`\tClaiming now...  taking pending`.green);
...
async function addCompensationPool() {
  console.log("\n\towner is going to add compensation pool".yellow);
  tx = farm.connect(owner).addCompensationPool(10, cCrss.address);
  let pid;
  (await tx).wait();
  pid = await farm.poolLength() - 1;
  console.log("\tCompensation Pid is now: %s".green, pid);
  await showPoolInfo(pid);

  return pid;
}

async function pauseCompensation() {
  console.log("\n\t%s is going to pause Compensation".yellow, owner.name);
  (await farm.connect(owner).pauseCompensation()).wait();
  console.log("\tCompensation Pid is now: %s".green, await farm.pid_compensation());
}

async function resumeCompensation() {
  console.log("\n\t%s is going to resume Compensation".yellow, owner.name);
  (await farm.connect(owner).resumeCompensation()).wait();
  console.log("\tCompensation Pid is now: %s".green, await farm.pid_compensation());
}

describe(
    "====================== Stage 5: Test CrossFarm ======================\n".yellow,
    async function () {
      ...
      it("How the initial empty Crss staking pool works with rewards.\n".green, async function () {
        ...
        await compoundAccumulated(0, bob);
        await harvestAccumulated(0, carol);
      });
      ...
      it("How the initial empty Crss/Bnb staking pool works with rewards.\n".green, async function () {
        ...
        await mintBlocks(10);
        await switchCollectOption(1, alice, "OnOff");

        await deposit(1, alice, 15);
        await withdraw(1, alice, 20);
        ...
        await switchCollectOption(1, bob, "OnOn");

        await deposit(1, alice, 10);
        await deposit(1, alice, 0);
        await deposit(1, alice, 15);
        await withdraw(1, alice, 10);
      ...
      it("Compound accumulated rewards.\n".green, async function () {
        await compoundAccumulated(1, alice, "Alice");
      ...
      it("Round 1. Alice, Bob, and Carol swapped a big amount of Bnb for Crss, and staked the Crss tokens.\n".green, async function () {
        ...
        await showPoolInfo(1);
        await showUserInfo(1, alice);
        ...
        await showPoolInfo(2);
        await showUserInfo(2, alice);
      ...
      it("MassHarvest, MassStake, MassCompound.\n".green, async function () {
      ...
      it("Compensation test.\n".green, async function () {
        pid_compensation = await addCompensationPool();

        let mints = 100;

        await mintBlocks(mints);
        await harvestAccumulated(pid_compensation, alice);
        await harvestAccumulated(pid_compensation, bob);
        await harvestAccumulated(pid_compensation, carol);
        await mintBlocks(mints);
        await harvestAccumulated(pid_compensation, alice);
        await harvestAccumulated(pid_compensation, bob);
        await harvestAccumulated(pid_compensation, carol);

        await pauseCompensation(); //----------------------------
      ...
      // it("deposit expected to be reverted.\n".green, async function () {
      //   await deposit(pid_compensation, alice, 0.5, true);
      // });
      // it("withdraw expected to be reverted.\n".green, async function () {
      //   await withdraw(pid_compensation, bob, 0.5, true);
      // });
      // it("switchCollectOption expected to be reverted.\n".green, async function () {
      //   await switchCollectOption(pid_compensation, bob, 'OnOn', true);
      // });
      // it("vestAccumulated expected to be reverted.\n".green, async function () {
      //   await vestAccumulated(pid_compensation, bob, true);
      // });
      // it("compoundAccumulated expected to be reverted.\n".green, async function () {
      //   await compoundAccumulated(pid_compensation, bob, true);
      // });
      // it("addCompensationPool expected to be reverted.\n".green, async function () {
      //   pid_compensation = await addCompensationPool();
      // });
